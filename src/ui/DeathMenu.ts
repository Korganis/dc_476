...
export default class DeathMenu extends UIMenu {
...
class deathLayer extends UILayer {
    constructor(game: Game) {
        super([
            new deathText(game),
        ]);
...
class deathText implements UIElement {
    pos: Vec2;
    size: Vec2;
    ...
    public draw(ctx: CanvasRenderingContext2D) {
        ...
        ctx.fillStyle = `rgba(255, 0, 0, ${this.counter})`;
        ctx.font = "80px Arial";
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText("You Died", this.size.x / 2, this.size.y / 2);
